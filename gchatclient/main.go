...
import (
	...
	"os"
	"time"
	//"gotalk/gotalkConfigurator" (needs to be built)
)

func main() {
	rand.Seed(time.Now().UnixNano())
	name := fmt.Sprintf("Anonymous%d", rand.Intn(400))
	fmt.Println("Starting gChatClient....")
	...
	gotalkConfigurator.GetConfiguration(gotalkConfigurator.CUSTOM,&confStruct,"chat.conf")
